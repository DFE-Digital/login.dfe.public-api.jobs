...
const getToken = async (config, clientId, correlationId) => {
  ...
  if (clientId) {
    const applications = new ApplicationsClient(config.publicApi.applications, correlationId);
    ...
    secret = application.relyingParty ? application.relyingParty.api_secret : undefined;
    if (!secret) {
      throw new Error(`No api secret configured for ${application.id} (clientId ${clientId})`);
    }
  }
...
const process = async (config, logger, data) => {
  ...
  if (!callback) {
    return logger.info(`publicinvitationnotifyrelyingparty_v1 not called for invited user ${userId} as no callback was supplied`, { correlationId })
  }
  ...
  try {
    ...
    logger.error(`Error notifying rp - ${e.message}. (userId: ${userId}, sourceId: ${sourceId}, callback: ${callback})`, { correlationId });
